...
export class Agent {
	...
	private async runContinuation(): Promise<void> {
		if (this.activeRequestIdentity?.sessionId !== (this.sessionId ?? this.attributionSessionId)) {
			this.activeRequestIdentity = undefined;
		}
		this.activeRequestIdentity ??= this.createRequestIdentity();
		await this.runWithLifecycle(async (signal) => {
	...
	/** Apply the caller's message conversion without running transient context hooks. */
	async convertMessagesToLlm(messages: AgentMessage[] = this._state.messages): Promise<Message[]> {
		return this.convertToLlm(messages);
	}

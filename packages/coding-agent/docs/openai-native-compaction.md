...
Manual `/compact` and automatic threshold/overflow compaction use the standalone
`/responses/compact` route for direct OpenAI Responses and ChatGPT Codex endpoints.
The request names the active model; it is not an ordinary assistant turn asking
that model to write a prose summary. Internal compactor model selection is owned
by the service. Branch-navigation summaries are unchanged.

## Configuration

- Default request timeout: **300,000 ms (five minutes)**, covering response headers
  and the complete JSON body. Silent 90+ second compactions are allowed. This does
  not inherit normal streaming idle or WebSocket connection timeouts.
...
The entire returned output window is persisted in the compaction entry's
`details.openaiCompaction`. Every retained item and the encrypted compaction item
